...
class ExpandableTable {
    ...
    createDataRow(mpgTeam, index) {
        ...
        const lsTeam = liveStandings?.filter(item => item.teamNum === mpgTeam.teamNum).pop();
        ...
        const nextMatchToPlay = calendarDiv?.filter(item => item.isPlayed === false).slice().shift();
        if (nextMatchToPlay) {
            const teamNextMatch = nextMatchToPlay?.matches.filter(it => it[0] === mpgTeam.teamNum || it[1] === mpgTeam.teamNum).pop();
            console.log('teamNextMatch', teamNextMatch);
            const homePlayer = teamsOfDivision?.filter(it => it.teamNum === teamNextMatch[0]).slice().shift();
            const awayPlayer = teamsOfDivision?.filter(it => it.teamNum === teamNextMatch[1]).slice().shift();

            console.log(`${homePlayer.name} contre ${awayPlayer.name}`);            
        }
    ...
    createExpandedRow(mpgUser) {
        ...
        if (nextMatchToPlay) {
            ...
            td.innerHTML += `
                    <div>Prochain match : ${homePlayer.name} contre ${awayPlayer.name}</div>
            `;
        }
...
async function fetchTeamsOfDivision() {
    try {
        ...
        if(data?.error === 'championshipNotManaged') {
            ...
            teamsOfDivision = data.teams;
        }
    } catch (error) {
        contentDisplay(); // Afficher le contenu même en cas d'erreur
        showError(`Impossible de charger les équipes de la division : ${error.message}`);
...
async function fetchMatches() {
    try {
        ...
        teamsOfDivision = allTeams[0].teams;
        ...
        allMatches.forEach((matches, i) => {
            ...
            liveStandings = matches.liveStandings;
            calendarDiv = matches.calendar;
